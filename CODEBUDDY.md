...
新增的工具组件：
- **AngleCalculator**：统一角度、方向和距离计算
- **HealthSystem**：管理生命值、护盾和伤害处理
- **EntityManager**：模板化的实体列表管理器
- **SoundManager**：单例模式的音效管理器
- **TextureManager**：单例模式的纹理管理器

详见 `REFACTOR.md` 了解详细的重构方案和使用方法。